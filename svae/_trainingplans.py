...
class CLTrainingPlan(pl.LightningModule):
    """
    ...
    **loss_kwargs
    ...
    def __init__(
        ...
        generative_replay_dl,
        generative_replay_adata_manager,
        replay_adata_manager=None,
        device = 'cpu',
        lr: float = 1e-3,
        weight_decay: float = 1e-6,
#         replay_importance: float = 0.0,
        eps: float = 0.01,
        ...
        **loss_kwargs,
        ...
        super().__init__()
        self.module = module
        self.generative_replay_dl = generative_replay_dl
        self.generative_replay_adata_manager = generative_replay_adata_manager
        self.replay_adata_manager = replay_adata_manager
        self.devicetomove = device
        self.lr = lr
        self.weight_decay = weight_decay
#         self.replay_importance = replay_importance
        self.eps = eps
...
#         self.module.replay_importance = replay_importance
    ...
    def training_step(self, batch, batch_idx, optimizer_idx=0):
        ...
        # override forward call
        if self.module.learnable_actions:
            # generative replay requires a different set-up (e.g. non None label_keys), so should be kept separate from replay 
            sampler = RandomSampler(self.generative_replay_dl.dataset, replacement=True, num_samples=batch['X'].shape[0])
            grdl = AnnDataLoader(self.generative_replay_adata_manager, shuffle=False, 
                                 batch_size = batch['X'].shape[0], indices = list(sampler))

            batch_generative_replay = next(iter(grdl))

        
        if self.replay_adata_manager is not None:
            # replay is setup in the same way as input data, so can use model dataloader
            replay_indices = np.random.choice(self.replay_adata_manager.adata.n_obs, size=batch['X'].shape[0], replace=False)
            scdl = AnnDataLoader(
                self.replay_adata_manager, indices=replay_indices, batch_size=batch['X'].shape[0]
            )  
...
#         for key, val in batch_generative_replay.items():
#             batch_generative_replay[key] = val.to(self.devicetomove)
            
            
#         for key, val in batch_replay.items():
#             batch_replay[key] = val.to(self.devicetomove)
...
#         _, _, scvi_loss = self.forward(batch, loss_kwargs=self.loss_kwargs)
        if self.module.learnable_actions:
            sampler = RandomSampler(self.generative_replay_dl.dataset, replacement=True, num_samples=batch['X'].shape[0])
            grdl = AnnDataLoader(self.generative_replay_adata_manager, shuffle=False, batch_size = 1, indices = list(sampler))
            batch_generative_replay = next(iter(grdl))
        
        if self.replay_adata_manager is not None:
            replay_indices = np.random.choice(self.replay_adata_manager.adata.n_obs, size=batch['X'].shape[0], replace=False)
            scdl = AnnDataLoader(
                self.replay_adata_manager, indices=replay_indices, batch_size=1
            )
            batch_replay = next(iter(scdl))
